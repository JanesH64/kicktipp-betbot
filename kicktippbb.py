...
def parse_match_rows(browser: RoboBrowser, community, matchday = None, skipOdds = False):
    ...
    for row in rows:
        ...
        try:
            if not skipOdds:
                odds= row[4].get_text().replace('Quote: ', '').split(" / ")
            else:
                odds= [0, 0, 0]
...
def place_bets(browser: RoboBrowser, communities: list, predictor, override=False, deadline=None, dryrun=False, matchday=None):
    ...
    for com in communities:
        ...
        matches_reference = parse_match_rows(browser, com, matchday)
        matches = parse_match_rows(browser, "aehrenmaennerundco", matchday, True)
        ...
        for field_hometeam, field_roadteam, match in matches:
            ...
            match_reference = list(filter(lambda x: (x[2].roadteam == match.roadteam and x[2].hometeam == match.hometeam) , matches_reference))[0][2]
            homebet, roadbet = predictor.predict(match_reference)
            if not override and (input_hometeam_value or input_roadteam_value):
                print("{0} - skipped, already placed {1}:{2}".format(match_reference,
            ...
            if deadline is not None:
                if not is_before_dealine(deadline, match.match_date):
                    ...
                    print("{0} - not betting yet, due in {1}".format(match_reference,
                                                                     timedelta_tostring(time_to_match)))
                    continue

            
            print("{0} - betting {1}:{2}".format(match_reference, homebet, roadbet))
